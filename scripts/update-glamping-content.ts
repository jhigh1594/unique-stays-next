...
function textNode(t: string, format = 0) {
  return { mode: 'normal', text: t, type: 'text', style: '', detail: 0, format, version: 1 }
}

function para(t: string) {
  return { type: 'paragraph', format: '', indent: 0, version: 1, direction: 'ltr', children: [textNode(t)] }
}

function h2(t: string) {
  return { tag: 'h2', type: 'heading', format: '', indent: 0, version: 1, direction: 'ltr', children: [textNode(t)] }
}
...
function linkPara(before: string, linkText: string, url: string, after: string) {
  return {
    ...
    children: [
      textNode(before),
      { type: 'link', format: '', version: 1, fields: { url, newTab: false, linkType: 'custom' }, children: [textNode(linkText)] },
      textNode(after),
    ],
  }
}
...
function boldQuestion(question: string, answer: string) {
  return {
    type: 'paragraph', format: '', indent: 0, version: 1, direction: 'ltr',
    children: [textNode(question, 1), textNode(answer)],
  }
}
...
async function main() {
  ...
  for (const slug of STAY_SLUGS) {
    ...
    stays[slug] = result.docs[0].id as number
  }
  ...
  const content = {
    root: {
      ...
      direction: 'ltr',
      children: [
        h2('The Trip You Didn\'t Know You Were Planning'),
        ...
        embedBlock(stays['geodesic-dome-22acres-ga']),

        h2('Start with a Dome'),
        ...
        embedBlock(stays['basecamp-treeloft-mo']),

        h2('Or Start with a Treehouse'),
        ...
        embedBlock(stays['dreamy-yurt-steamboat-co']),

        h2('The Yurt Option: Maximum Atmosphere, Minimum Price'),
        ...
        embedBlock(stays['hobbit-house-fayetteville-ar']),

        h2('The One That Doesn\'t Fit a Category'),
        ...
        embedBlock(stays['glass-tiny-warren-vt']),

        h2('The Glass Tiny Home: Where Design Meets Mortgage'),
        ...
        embedBlock(stays['sauna-aframe-saugerties-ny']),

        h2('The A-Frame: Two Hours from Manhattan, Miles from Everything Else'),
        ...
        embedBlock(stays['silo-bitterroot-mt']),

        h2('The Wildcard: A Grain Silo in Montana'),
        ...
        h2('What to Know Before You Book Your First Glamping Trip'),
        ...
        boldQuestion('What should I pack for my first glamping trip?', ' Less than you think. Check the listing for what the host provides. Most glamping stays include linens, towels, and basic kitchen equipment. Focus on weather-appropriate clothing, a book, food you want to cook, and whatever you need for the specific activity (hiking boots, a swimsuit, a bottle of wine). The remote worker\'s guide to unique stays covers packing for work-friendly stays specifically.'),
        boldQuestion('How do I choose between a dome, yurt, treehouse, or something else?', ' Start with what draws you. If you want to look up and see sky, pick a dome. If you want the sound of wind through trees, pick a treehouse. If you want the lowest price with the most atmosphere, pick a yurt. If you want a story nobody else has, pick the silo. For deeper dives, see the treehouse rentals guide and the lakefront stays roundup.'),

        // Internal link paragraphs
        linkPara('See also: ', 'Best treehouse rentals in the USA', '/journal/best-treehouse-rentals-usa', '.'),
        linkPara('See also: ', 'Lakefront unique stays guide', '/journal/lakefront-unique-stays-water-cabins-domes-aframes', '.'),
        linkPara('See also: ', 'Remote worker\'s guide to unique stays', '/journal/remote-workers-guide-unique-stays', '.'),
      ],
    },
  }
  ...
  try {
    ...
    console.log('Content update succeeded')
  } catch (err: any) {
    console.error('Content update failed:', err.message)
    if (err.data?.errors) {
      console.error('Errors:', JSON.stringify(err.data.errors, null, 2))
    }
  }

  const verified = await payload.findByID({ collection: 'blog-posts', id: POST_ID, depth: 1 })
  console.log(JSON.stringify({
    id: verified.id,
    slug: verified.slug,
    status: verified.status,
    linkedStays: Array.isArray(verified.linkedStays) ? verified.linkedStays.length : 0,
    hasContent: Boolean(verified.content),
