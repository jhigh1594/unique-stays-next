...
function request(method: string, path: string, body?: unknown): Promise<any> {
  return new Promise((resolve, reject) => {
    ...
    const req = http.request(opts, (res) => {
      ...
      res.on('end', () => { try { resolve(JSON.parse(buf)); } catch { reject(new Error(buf.substring(0,200))); } });
    });
    req.on('error', reject);
...
function linkPara(before: string, linkText: string, linkUrl: string, after: string) {
  return {
    type: 'paragraph', format: '', indent: 0, version: 1, direction: 'ltr', textFormat: 0, textStyle: '',
    children: [
      text(before),
...
async function main() {
  ...
  // Update editorial fields + linked stays
  console.log('Updating editorial fields...');
  const r1 = await request('PATCH', `/api/blog-posts/${postId}`, {
    ...
    linkedStays: [8, 12, 42, 29, 136, 102, 83, 10, 122, 151],
  });
  console.log('Fields updated:', r1.id);

  // Update content
  console.log('Updating content...');
  const r2 = await request('PATCH', `/api/blog-posts/${postId}`, { content });
  console.log('Content updated:', r2.id);

  // Verify
  const v = await request('GET', `/api/blog-posts/${postId}?depth=0`);
  console.log('Verified:', v.id, v.status, v.slug, 'hasContent:', !!v.content);
}
