...
description: |
  Autonomous journal content pipeline for UniqueStaysUSA. Researches
  keywords, writes editorial content, publishes to Payload CMS, and
  tracks results. Use when creating journal posts, writing blog content,
  planning content strategy, or the user says "next post", "write an
  article", "journal", "content sprint", "publish", or anything about
  creating travel editorial content. Also triggers on "content calendar",
  "what should we write next", or "run the pipeline".
---
...
- No banned words (stunning, breathtaking, magical, life-changing, perfect, amazing, incredible, unforgettable, cozy, hidden gem, wanderlust, bucket list, must-see, nestled, escape the everyday)
- No exclamation marks
- No em dashes — rewrite using periods, commas, colons, or separate sentences. Maximum zero per article.
...
Optimize for search and AI citation.
...
Quality gate. Score against the rubric.
...
7. **Distinct voice check** — verify no two stay descriptions sound alike
...
Publish directly to Payload CMS.
...
1. Read `scripts/ralph/prd.json` — what's the current sprint and story?
2. Read `scripts/ralph/progress.txt` — what patterns have been learned?
3. Find the highest-priority story where `passes: false`
4. Execute that story's phase
5. Update `prd.json` with `passes: true`
6. If all stories pass, start a new sprint (return to Phase 1)

### Stop conditions

- All calendar entries for the current month are completed
- Quality gate fails 3 times on the same article (escalate to user)
- User explicitly cancels or says "stop", "pause", or "that's enough for now"
...
- Updating existing published posts (use direct Payload PATCH)
